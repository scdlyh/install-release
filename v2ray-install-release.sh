...
## 处理参数的演示函数
judgment_parameters() { VERSION=v4.45.2
  while [[ "$#" -gt '0' ]]; do
    case "$1" in
      '--remove')
        if [[ "$#" -gt '1' ]]; then
...
get_version() {
  ...
  # 获取V2Ray发布版本号
  TMP_FILE="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$TMP_FILE" 'https://hubapi.pureth.tk/repos/v2fly/v2ray-core/releases/latest'; then
    "rm" "$TMP_FILE"
...
download_v2ray() {
  DOWNLOAD_LINK="https://ghproxy.com/https://raw.githubusercontent.com/scdlyh/Xray-install/main/v2ray-install-release.sh/https://github.com/v2fly/v2ray-core/releases/download/$RELEASE_VERSION/v2ray-linux-$MACHINE.zip"
...
install_v2ray() {
  # 将V2Ray二进制文件安装到/usr/local/bin/和$DAT_PATH
  install_file v2ray
  ...
  install -d "$DAT_PATH"
  # 如果文件存在，那么将不会安装或更新geoip.dat和geosite.dat
...
main() {
  ...
  # 如果文件存在，则不显示安装或更新geoip.dat和geosite.dat的内容输出
  if [[ ! -f "${DAT_PATH}/.undat" ]]; then
